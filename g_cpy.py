...
class Ui_win():
    def setupUi(self, win):
        ...
        self.listWidget_R.setCurrentRow(2)
        ...
        self.CD.setup(self)
        self.CD.setup2(self)
        self.CD.model().rowsInserted.connect(lambda: self.edit_table(self.data,self.filt_table))
        self.CD.model().rowsRemoved.connect(lambda: print('signal active....'))
        #self.FW = filter_win()
        #self.FW.setup2(self)
        ...
        self.pushButton_3.setGeometry(QtCore.QRect(660, 800, 121, 31))
        self.pushButton_3.setObjectName("pushButton_3")
        self.pushButton_3.clicked.connect(self.clear_lb)

        self.unionButton = QtWidgets.QPushButton(self.centralwidget)
        self.unionButton.setGeometry(QtCore.QRect(660, 840, 121, 31))
    ...
    def retranslateUi(self, win):
        ...
        self.pushButton_3.setText(_translate("win", "Clear"))
...
#################################### current .
    ...
    def import_file(self):
        ...
        if self.fname:
            ...
            self.md5_fname = manager.md5_convert(self,self.fn)
    ...
    def edit_table(self,data,table):
        #set up for gb func
        typer_col = self.CD.get_type()
        typer_row = self.RD.get_type()
        dl = self.dimension_header

        rr = self.RD.get_roller()
        cr =self.CD.get_roller()
        ...
        pd_gd = pd.DataFrame(gb[self.head_data_list].head(-1))
        Model = PandasModel(pd_gd)
        table.setModel(Model)
        
            
        print('list widget signal activated...........')
    ...
    def plot_graph(self):
        ...
        self.gb = manager.gb(self,self.typer_row,self.typer_col,self.dl,self.rr,self.cr,self.data)
        print(self.gb)
        print('**************')
        self.filt_data = manager.query_data(self,self.typer_col,self.typer_row,self.rr,self.cr,self.dimension_header,self.measure_header,self.gb)
